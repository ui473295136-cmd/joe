...
(async()=>{
  ...
  try{
    ...
    await page.waitForFunction(()=>/我应付给\s*辉子/.test(document.querySelector('#settlementLines')?.innerText||''),{timeout:5000});
    ...
    await page.waitForSelector('.cw-quick-chip[data-exp-cat="油费"]',{timeout:7000});
    ...
    await page.waitForFunction(()=>/¥371\.75/.test(document.querySelector('#settlementLines')?.innerText||''),{timeout:5000});
