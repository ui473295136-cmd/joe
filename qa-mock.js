...
const profiles=TEAM.map(person=>({trip_slug:'chuanxi2026',person,nickname:person+'昵称',role:{瑞子:'酒店 / 账本',普子:'攻略 / 路况',航子:'机票 / 航班',辉子:'租车 / 车务'}[person],last_seen:now,updated_at:now,avatar_url:null}));
...
window.fetch=async(input,init={})=>{const url=String(typeof input==='string'?input:input?.url||'');if(url.includes('/functions/v1/trip-sync')){let b={};try{b=JSON.parse(init.body||'{}')}catch(e){}const a=b.action;if(a==='get_state')return res(baseState);if(a==='heartbeat'||a==='update_person_position'||a==='acknowledge_expense'||a==='update_profile'||a==='upload_avatar'||a==='add_expense'||a==='update_expense'||a==='delete_expense'||a==='add_repayment'||a==='update_repayment'||a==='delete_repayment'||a==='attach_receipt')return res({ok:true,row:{id:'qa-row'}});return res({ok:true})}if(url.includes('/functions/v1/trip-trash')){let b={};try{b=JSON.parse(init.body||'{}')}catch(e){}if(b.action==='list_trash')return res(trash);if(b.action==='restore_expense'){trash.expenses=[];return res({ok:true,row:{id:'qa-old-exp'}})}if(b.action==='restore_repayment')return res({ok:true,row:{id:'qa-old-rp'}});return res({ok:true})}return realFetch(input,init)};
