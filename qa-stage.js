...
async function open(browser,date,time='09:00'){
  ...
  await page.waitForFunction(()=>document.documentElement.classList.contains('cw-itinerary-admin-ready'),{timeout:12000});
  return page;
}
(async()=>{
  ...
  try{
    ...
    await p.waitForFunction(()=>/暂无可靠天气预报/.test(document.querySelector('#cwHourlyRouteWeather')?.innerText||''),{timeout:12000});
    ...
    if(!head.includes('雅安 → 泸定 → 康定 → 折多山 → 新都桥')||!head.includes('280.0km')||!head.includes('7小时15分')||!head.includes('上喜民宿（新都桥店）'))throw new Error('TODAY_HEADER_FAIL '+head);
    ...
    if(!head.includes('新都桥 → 塔公 → 八美 → 丹巴中路藏寨')||!head.includes('160.0km')||!head.includes('4小时30分')||!head.includes('谷道山居（中路藏寨店）'))throw new Error('SWITCH_HEADER_FAIL '+head);
