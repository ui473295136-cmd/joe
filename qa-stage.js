...
(async()=>{
  ...
  try{
    ...
    await p.waitForFunction(()=>/10月03日 · 今日行程/.test(document.querySelector('#view-today .page-title')?.innerText||''),{timeout:4000});
    ...
    await p.waitForFunction(()=>/10月04日 · 后续行程/.test(document.querySelector('#view-today .page-title')?.innerText||''),{timeout:4000});
