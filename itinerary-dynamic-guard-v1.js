(() => {
  ...
  function header() {
    ...
    if (!api || !box) return;
    const i = Number(box.querySelector(".day-chip.on")?.dataset.day || 0), d = api.getDay(i);
    if (!d) return;
    const title = $("#view-today .page-title h1"), p = $("#view-today .page-title p");
    if (title) title.textContent = `${String(d.day_date).slice(5).replace("-", "月")}日 · ${d.day_date === new Date().toLocaleDateString("sv-SE", {timeZone:"Asia/Shanghai"}) ? "今日行程" : "行程计划"}`;
