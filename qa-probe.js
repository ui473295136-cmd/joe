...
async function run(){
  ...
  await waitFor(()=>!$('#app')?.hidden,5000);await waitFor(()=>$('#nextTripCard')&&$('#simpleItinerary')&&$('#identitySwitcher'),9000);
  ok('页面启动',!!$('#app')&&!$('#app').hidden);
  ok('身份标题',document.title.includes(ME),document.title);
  ok('底部导航5项',$$('#bottomNav button[data-view]').length===5);
  ...
  ok('现在页地图缩略隐藏',$('#mapCard')?.classList.contains('minimal-hidden'));
  ...
  ok('昵称同步到身份卡',$(`#identityGrid [data-switch-person="${ME}"]`)?.innerText.includes('昵称'));
