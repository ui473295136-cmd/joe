...
`;
  ...
  document.addEventListener(
    "click",
    (event) => {
      if (event.target.closest?.("[data-itin-edit]")) setTimeout(enhanceSave, 0);
    },
    true,
  );
  ...
  window.CWItinerarySaveTop = { enhance: enhanceSave };
  ensureSaveStyle();
  enhanceSave();
})();